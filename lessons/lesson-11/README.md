...
| 文件 | 角色 | 用途 |
| --- | --- | --- |
| [handout.md](./handout.md) | 现行·学生正式讲义 v0.3.1 | 聚焦固定指标、预算、停止与回退；区分冻结的批量执行队列与优化循环；使用人工基准评价限定条件遗漏率 |
| [teaching-plan.md](./teaching-plan.md) | 现行·教师教案 v0.4.1 | 整数时长的 90 分钟流程、分段实践与中点点评、三种 trace 演示与备用路径 |
| [slides.md](./slides.md) | 现行·逐页母稿 v0.5.0（20 页） | 逐页屏显文案、对象化视觉结构、讲述备注、互动、整数时长与来源边界 |
