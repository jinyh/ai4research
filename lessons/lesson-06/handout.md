...
适用课次：第 6 课
文档类型：学生正式讲义
状态：现行；问题门九项条件、第 3-6 课累计路径、贯穿案例与工件落点已对齐
变更记录：
- v1.2.1 (2026-09-12): 将 MI 材料明确降为课后迁移阅读，不再作为第 7-9 课主线输入；课堂和跨课接口只使用结构化阅读卡贯穿案例
- v1.2.0 (2026-09-12): 聚焦“候选题取舍→问题边界→一个可检验前提”；新颖性改为可记录的诊断项，不作为问题门硬条件；竞争假设只留替代解释标记，完整设计移交第 7 课；重写课堂最低产出、课后补全与研究门边界
- v1.1.1 (2026-08-23): 门条件自查清单后补充 `failure-log.md` 创建位置与脚手架 §7 空表指引；不改变九项门条件与课堂结构
...
6. 用意义、可行性和兴趣完成候选题取舍；如有依据，再记录问题新、视角新或方法新的线索；
7. 对照问题门条件完成提交前自查。
...
- **当前解释**：阅读卡强制定位原文，减少把模型推断误写成原文结论的情况，从而降低遗漏率。
- **替代解释标记**：评价者看到阅读卡字段提示后更容易标出限定条件，表面差异可能来自标注提示，而非摘要本身保留得更多。

把这条标记作为第 7 课的输入即可。第 7 课再正式编号 H1/A1/A2/A3，并设计可区分结果；本课不提前完成那一讲的工作。
...
| --- | --- | --- | ---: | --- | --- |
| 第 3 课 | 1 条可复盘检索记录 + 2 条来源审计 + 至少 1 条 `verified` 或带限制的候选文献 | 复用同一检索式和纳入/排除条件，把候选表扩展到至少 6 篇 | 20-30 分钟 | `notes/literature-search.md` 或 `evidence-map.md` 的候选表 | 不单独提交；第 6 课问题门统一检查 |
...
| 第 6 课 | 选定一个候选题，写清边界、一个可证伪命题、一个待验证前提和一个替代解释标记；九项门条件标注状态 | 只补自查中的缺口，不重做已通过的工件 | 按缺口而定 | `problem-definition.md` + `failure-log.md` + 上述持续工件 | 课后提交一次问题门材料包；完整竞争假设设计进入第 7 课 |
...
`failure-log.md` 在问题门首次成为必查工件：如果第 1-5 课还没有建过它，现在在个人项目中创建，空表见[早期研究脚手架](../../course/starter-template.md) §7 轻量失败记录。
...
完整问题定义见 [mi-problem-definition-demo.md](./mi-problem-definition-demo.md)。它把第 5 课“patching success 不等于 causal faithfulness”的证据缺口拆成三个命题：效率 `H-eff`、稳健性 `H-rob` 和忠实性 `H-faith`，并明确非例、推翻条件和五项必要条件。该材料只用于课后迁移练习，不在课堂展开，也不作为第 7–9 课的贯穿案例或必备输入。

![从问题门到 baseline](assets/mi-question-to-baseline.svg)

*图注：课程自绘迁移示例，不表示实验已运行，也不替代阅读卡贯穿案例。来源与许可见 [assets/README.md](./assets/README.md)。*
...
2. 把它与你的文献空白候选一起放入候选题池，用四维选题标准卡（意义、新颖性、可行性、兴趣）比较，写出选定哪个、为什么。新颖性只在有依据时记录类别和待核验状态，不作为淘汰硬门。
