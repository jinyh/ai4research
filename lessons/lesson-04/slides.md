...
- v0.9.1 (2026-09-12): 按视觉准则 v1.5.0 把原文、阅读卡和证据阶梯提升为主对象；标题 30–32pt、常规正文 22–24pt、原文/工作表 18–20pt，正文改以 regular 为主。P11 改用 Keshav p.83 §2 真实原文裁图与一项划读任务，P15 改为两栏阅读卡工作表并移除屏显计时。页数、屏显语义与 90 分钟结构不变。
- v0.9.0 (2026-09-12): 减负轮将课堂主线聚焦为一条关键主张的原文位置—证据—判断；方法与演示各 15 分钟，P15 个人实践改为 12+10 分钟并插入 5 分钟教师点评；全部逐页时间改为整数时长；页数不变，PPTX 待同步。
...
- 本轮内容、节奏和映射已同步；现有 PPTX 仍对应上一轮内容，待内容集成后重建并重走三重检查。视觉规则见 [ppt-quality-gates.md](../../course/ppt-quality-gates.md) 与 [ppt-design-criteria.md](../ppt-design-criteria.md)；
...
| 时长 | 页码 | 环节 | 课堂结果 |
| --- | --- | --- | --- |
| 5 分钟 | P01-P02 | 回扣第 3 课：入口核验之后，关键主张仍可能被错读 | 精读方法入口 |
| 5 分钟 | P03-P04 | 本课目标与"读懂摘要但误解贡献" | 目标共识 |
| 15 分钟 | P05-P10 | 方法：五步流程和主张核验五项，只围绕一条关键主张 | 主张精读方法链 |
| 15 分钟 | P11-P13 | Keshav 定位校准 → C02 主张 → C16/C31 收窄结论 | 一条可复盘 trace |
| 5 分钟 | P14 | 实践说明：任务、模板与备用路径 | 选择论文进入实践 |
| 12 分钟 | P15 | 个人实践一：定位原文并写出证据与限定 | 带位置的主张记录 |
| 5 分钟 | P15 | 教师中点点评：作者主张、AI 转述与个人判断分层 | 越级推断校正 |
| 10 分钟 | P15 | 个人实践二：完成边界、偏差审计和 AI 记录 | 一张完整阅读卡 |
| 10 分钟 | P16 | 两人互查：主张有位置、AI 与原文分栏、偏差审计可复查 | 反馈记录 |
| 5 分钟 | P17 | 本讲知识点总结 | 主链与边界回收 |
| 3 分钟 | P18 | 退出卡与第 5 课预告 | 卡点与需求 |

> 节奏对齐 `备课规划.md` v2.9.0 的减负与时长制口径。单段个人实践不超过 15 分钟，合计恰好 90 分钟，不写绝对起止时间。
...
偏差审计记录的是可复查的偏差，是后续评价 AI 辅助阅读工作流的重要证据。第 3 课的来源审计是入口核验，本课是内容核验——一条论文可以 身份已核验（存在且 DOI 正确，但拟用主张仍须对回原文），但 AI 对它的复述仍可能错读、过度概括或位置错指。偏差审计表记录的就是这种内容层面的偏差。未发现明显错误时，记录检查过的风险点及核验依据，不虚构错误。
...
Keshav p.83 §2 真实原文裁图占主画面；右侧只放两条课堂构造 AI 输出和原文核验，底部保留一个划读动作与可见短来源。
...
说明任务后切换至 P15 模板停留页。教师巡视只处理共同卡点，不逐台排障。模型不可用者使用纸面或 Markdown 模板完成。
...
两栏阅读卡工作表：左侧来源、作者主张与原文证据，右侧局限、AI 建议、偏差审计与内容状态；完整条件与写入位置邻接，不显示课堂分钟数。
...
第一段完成主张定位与证据限定；教师插入“作者主张、AI 转述和个人判断”分层点评；第二段补全边界、偏差审计和 AI 使用记录。
