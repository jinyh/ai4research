...
变更记录：
- v1.5.1 (2026-09-16): 按教师“通过，修改后续课程和PPT”确认推进；统一机制、可复算案例及真实日志字段，修复逐页映射，保持既定目标与 90 分钟安排。
- v1.5.0 (2026-09-12): 减负轮将课堂主线固定为一次可复盘的“问题 → 检索式 → 正式来源 → 一条拟用主张 → 候选表”；明确课堂最低产出、课后扩展和第 6 课问题门累计条件的边界。
- v1.4.0 (2026-08-14): 新增三处呈现增强——§二·5 检索流程的人机分工表、§三 角色转换路径、§五 线索表升级到候选表的条件表；四态口径、三件套规则、案例与来源口径未变，学习目标与 90 分钟课堂结构未变；与 mi-search-trace-demo.md v0.5.1、teaching-plan.md v1.3.0 同步
- v1.3.1 (2026-08-09): 原"练习 5：27 分钟课堂最低产出"改名为独立的"个人实践最低产出"小节并去除时间标注，与练习 1-4 的随堂小练习分开呈现；呈现增强，规则、产出数量与时间结构不变
...
教师 MI 检索 trace 是这套分工的真实实例：agent 起草了分支框架、词频校验、检索式、执行与身份核对，其凭记忆给出的 11 个锚点中 6 个身份不符被当场抽查发现；教师质疑后又有基准集自评轮暴露命中率 4/22，补入元层分支与双轨检索（见 [mi-search-trace-demo.md](./mi-search-trace-demo.md) §3、§6、§10）。分工的含义不是"AI 不可信所以全都自己做"，也不是"AI 可以全自动"，而是：**AI 出草稿与线索，人做质疑与决定，且全程留可复盘记录**。
...
1. **问题收敛**：出现明确的参照问题或拟用主张。没有参照物时，角色只能标"待问题收敛"；
2. **拟用主张层核验**：至少一条拟用主张回原文核对一致，核验状态升到 `verified` 或 `verified-with-caveat`（§四·3）；
3. **AI 起草建议**：AI 可以给出角色建议与理由，但建议本身只是线索；
4. **人工确认改标**：人工确认参照问题与理由成立后，角色才能改标。

本课先标"预期证据角色"，第 4-5 课经精读和证据整理后可按此路径修正角色。当前课程材料尚无"探索线索提升为主证据"的完整真实示例；教师 MI trace 的 36 条线索因研究问题未收敛而全部停留在探索线索（见 [mi-search-trace-demo.md](./mi-search-trace-demo.md) §5），这本身就是纪律演示：**没有参照问题，不指认角色**。
