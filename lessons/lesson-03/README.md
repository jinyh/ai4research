...
| [mi-search-trace-demo.md](./mi-search-trace-demo.md) | 现行·教学资产 v0.2.1 | MI 宽泛主题检索真实 trace：分支确定方法审计、17 条待筛选线索、4 条幻觉引用 rejected 实例；v0.2.0 补执行 B7-B9 分支检索并新增跨源核验（OpenAlex/dblp）与单源局限记录，第一版原文保留作记录演进教学点；讲义 §六 检索质量自评的分支审计真实示例；方向收敛段待真实方向指认后升版 |
