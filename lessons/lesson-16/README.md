...
| 文件 | 角色 | 用途 |
| --- | --- | --- |
| [handout.md](./handout.md) | 待定稿·学生讲义 v0.1.0 | 内容完整但 front matter 仍标“草稿，待教师复核与第二轮审校”；完成教师复核前不得视为正式发布版 |
| [teaching-plan.md](./teaching-plan.md) | 现行·教师教案 v0.3.1 | 90 分钟流程、PPT 执行索引、演示脚本、课堂产出验收、讲后复盘；正式内容门待讲义复核 |
| [slides.md](./slides.md) | 现行·逐页母稿 v0.3.1（20 页） | 逐页屏显文案、视觉结构、讲述备注、互动、时间、来源与事实边界；PPT 技术完成不等于整课正式发布 |
| [keystone-design-spec.md](./keystone-design-spec.md) | 现行·关键页设计规格 v1.0.1 | 10 个关键页的视觉、教学与验收契约 |
| [slides.pptx](./slides.pptx) | 技术检查完成·课堂 PPT 候选（20 页） | 封面只保留正式课名；P18 为独立知识点总结；逐页含 `[Sources]` speaker notes；待内容门解除后确认正式发布状态 |
...
- 门 1（课次目标）、门 3（90 分钟教学）和门 4（逐页映射）沿用既有检查结果。
- 门 2 只达到**草稿内容检查完成、待教师复核**；由于 `handout.md` 明确标为草稿，不得登记为正式内容门通过。
- 门 5（PPT 制作）和门 6（三重检查）的技术结果有效，但不替代门 2；`slides.pptx` 只能作为待内容定稿的课堂课件候选。
- 正式发布阻塞于教师对讲义、贯穿案例虚构数值、演示工件和班级人数方案的复核。若复核导致内容变化，再重走受影响的内容、映射与 PPT 检查；若只确认现有内容，可补复核记录后解除阻塞。
- 下表保留 2026-08-07 的原始检查记录用于追踪，其中“门 2 通过”和“门 7 无需归档”不再代表当前正式发布状态。
