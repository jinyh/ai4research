...
> 本文件只给出 AI 建议，不修改也不替代[现行 trace](../../../lessons/lesson-03/mi-search-trace-demo.md)。现行 trace 仍是题名、作者、年份、稳定来源、检索过程与正式核验状态的唯一事实源。本文件冻结于原 trace v0.5.1 的所列 SHA-256 快照；本文件中的问题、角色、筛选动作、核验状态与允许主张全部保持 `awaiting-human`。研究负责人确认前，不得写回正式候选文献表、精读卡或证据地图。
